...
contract CertificateOfParticipationNFT is ERC721URIStorage {
    ...
    constructor() ERC721("Certificate Of Participation for Developer Training", "COPDT") {
    }

    function generateTokenURL(string memory participentName) internal pure returns (string memory){
        string memory imageURL = 'ipfs://QmTBTbrSSaXAqWiHuj1Lw2erWjArFvGw3ZrmCKfC8EZk3n';
        string memory empty = '';
        string memory json = '';

        if (keccak256(bytes(participentName)) == keccak256(bytes(empty))) {
            json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Smart Contracts: Developer Training", "description": "This NFT certifies that the original holder joined the Smart Contracts Developer Training session conducted by OrangeHRM", "image_data": "', imageURL, '"}'))));
        } else {
            ...
            json = Base64.encode(bytes(string(abi.encodePacked('{"name": "Smart Contracts: Developer Training", "description": "This NFT certifies that ', participentName ,' joined the Smart Contracts Developer Training session conducted by OrangeHRM", "image_data": "', imageURL, '"}'))));
        }
    ...
    function getCertificate(string memory participentName) public {
        // prevent minting after 1654596348 unix time
        require(block.timestamp < 1654596348, 'Time to mint the NFT is over');
